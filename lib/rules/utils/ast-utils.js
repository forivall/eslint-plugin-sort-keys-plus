 ...
 * @param {import('eslint').Scope.Reference[]} references The array that the reference belongs to.
...
function isLogicalIdentity(node, operator) {
  switch (node.type) {
    case 'Literal':
      return (
        (operator === '||' && getBooleanValue(node) === true) || (operator === '&&' && getBooleanValue(node) === false)
      )
